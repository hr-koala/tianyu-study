...
method.scroll = function() {
    ...
    eleScroll.on('mousedown touchstart', function(e) {
        ...
        lastY = e.clientY || e.originalTarget.targetTouches[0].clientY
        flag = (function() {
    ...
    }).on('mousemove touchend', function(e) {
        if (flag && eleScroll[0].scrollHeight > eleScroll[0].clientHeight) {
            var curY = e.clientY || e.originalTarget.targetTouches[0].clientY;
...
method.callVirtualKeyboard = function(callback) {
    ...
    $('.complie').on('click', 'textarea', function(){
        callback.call(this);
    }).on('click', 'input[type="text"]', function(){
        callback.call(this);
    })
    $('.complie').on('click', '.box-txt', function(){
        callback.call(this);
    })
    $('.form-text').on('click', function(){
        callback.call(this);
    })
    $('.add-img').on('click', '.area-txt', function(){
        callback.call(this);
    })
}
...
method.cleanFocus = function() {
    ...
    var selectors = [
        ".complie textarea", '.form-text', '.complie input'
    ].join(",");
